...
# 为了构建字符级语言模型来生成新的名称，你的模型将学习不同的名称模式，并随机生成新的名字。
import numpy as np
import random
import time
import cllm_utils  # 提供前向反向传播等函数
...
def sample(parameters, char_to_ix, seed):  # 根据RNN输出的概率分布序列对字符序列进行采样
    """
    参数：
...
def optimize(X, Y, a_prev, parameters, learning_rate=0.01):   # 执行训练模型的单步优化。
    """
    参数：
...
def model(data, ix_to_char, char_to_ix, num_iterations=3500,
          n_a=50, dino_names=7, vocab_size=27):   # 训练模型并生成恐龙名字
    """
    参数：
        data -- 语料库
        ix_to_char -- 索引映射字符字典
...
# print(chars)
# print("共计有%d个字符，唯一字符有%d个"%(data_size,vocab_size))  #共计有19909个字符，唯一字符有27个
